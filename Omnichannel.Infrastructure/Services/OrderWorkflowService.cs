    ...
    public class OrderWorkflowService : IOrderWorkflowService
    {
            ...
            string cartId, CheckoutViewModel model, string? customerUserId = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var allocationPlan = new List<(string ProductId, decimal Price, int BatchId, int Qty)>();

                // 1. Kiểm tra tồn khả dụng và lập kế hoạch khóa tạm (Soft Reserve) theo FEFO
                foreach (var item in cart.Items)
                {
                    var allocatedBatches = await _invEngine.AllocateBatchesFEFOAsync(store.StoreId, item.ProductId, item.Quantity, cancellationToken);
                ...
                {
                    OrderId = newOrderId,
                    OrderChannel = 2, // 2: Website Storefront
                    ...
                    PaymentStatus = 1, // 1: Chờ thanh toán
                    OrderStatus = 2,   // 2: Đã khóa tồn (Soft Reserved)
                ...
                foreach (var alloc in allocationPlan)
                {
                    var inventory = await _context.StoreInventories
                        .FirstOrDefaultAsync(si => si.StoreId == store.StoreId && si.BatchId == alloc.BatchId, cancellationToken);
                ...
                var vietQr = GenerateVietQr(newOrderId, order.FinalTotal);
            ...
            BankCallbackDto callback, CancellationToken cancellationToken = default)
        {
            var order = await _context.Orders
                ...
                .FirstOrDefaultAsync(o => o.OrderId == callback.OrderId, cancellationToken);
            ...
            try
            {
                ...
                order.PaymentStatus = 2; // 2: Đã thanh toán
                order.OrderStatus = 5;   // 5: Hoàn tất
                ...
                {
                    var inventory = await _context.StoreInventories
                        .FirstOrDefaultAsync(si => si.StoreId == order.StoreId && si.BatchId == detail.BatchId, cancellationToken);
                    ...
                    if (inventory != null)
                    {
                        ...
                        var log = new InventoryLog
                        {
                            ...
                            TransactionType = 3, // 3: Bán Online
                            ReferenceDocNo = order.OrderId,
                            QuantityChange = -detail.Quantity,
                            RemainingQuantity = inventory.PhysicalQuantity,
                            CreatedBy = order.CustomerId ?? "SYSTEM-WEB",
                            CreatedAt = DateTime.UtcNow,
                            Notes = $"Xuất kho hoàn tất đơn hàng Online {order.OrderId}"
                        };
                ...
                decimal totalBeforeTax = Math.Round(order.FinalTotal / 1.1m, 2);
                decimal vatAmount = order.FinalTotal - totalBeforeTax;

                var invoice = new Invoice
                {
                    InvoiceId = $"INV-{DateTime.UtcNow:yyyyMMdd}-{Guid.NewGuid().ToString("N")[..6].ToUpperInvariant()}",
                    OrderId = order.OrderId,
                    InvoiceSeries = "1C26TAA",
                    InvoiceNumber = DateTime.UtcNow.Ticks % 10000000,
                    TaxLookupCode = Guid.NewGuid().ToString("N")[..12].ToUpperInvariant(),
                    TotalBeforeTax = totalBeforeTax,
                    VatRate = 10.00m,
                    VatAmount = vatAmount,
                    TotalAfterTax = order.FinalTotal,
                    IssuedAt = DateTime.UtcNow,
                    IssuedByStaffId = "USR-SUPERADMIN-0001",
                    IsCancelled = false
                };
                ...
                await transaction.CommitAsync(cancellationToken);
                _logger.LogInformation("Đã chốt trừ tồn kho và phát hành hóa đơn {InvoiceId} cho đơn hàng {OrderId}.",
        ...
        public async Task<bool> CancelOrderAndReleaseReserveAsync(string orderId, string reason, CancellationToken cancellationToken = default)
        {
            ...
            if (order == null || order.OrderStatus == 5 || order.OrderStatus == 6)
            {
                return false;
            }
